...
import { addPersons, attachTags, type Db, type EntityStamp } from './lib/entities';
import {
	openRun,
	closeRun,
	emitSource,
...
export async function run(db: Db, opts: ImporterRunOptions = {}): Promise<ImporterSummary> {
	...
	return summarize('grammar', stats, drifted, { records: records.length });
}
