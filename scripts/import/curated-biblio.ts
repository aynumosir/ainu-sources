...
export async function run(db: Db, opts: ImporterRunOptions = {}): Promise<ImporterSummary> {
	...
	if (!fs.existsSync(CURATED_BIBLIO_FILE)) {
		throw new Error(`curated biblio file not found: ${CURATED_BIBLIO_FILE}`);
	}
	...
	console.log(
		`${DRY_RUN ? '[DRY-RUN] ' : ''}import:curated-biblio  (${slice.length}/${entries.length} entries)`
	);
	...
	return summarize('curated-biblio', stats, drifted, { entries: slice.length });
}
