...
export type ReviewedPerson = { slugs: string[]; name: string; acceptedNames?: string[]; acceptedPriorValues?: Partial<Record<Column, (string | number | null)[]>>; expected: ReviewPatch; corrected: ReviewPatch };
	...
	for (const entry of entries) {
		for (const row of rows.filter((r) => entry.slugs.includes(r.slug))) {
			...
			for (const [key, value] of Object.entries(entry.corrected)) {
				...
				if (row[col] !== entry.expected[col] && row[col] !== value && !entry.acceptedPriorValues?.[col]?.includes(row[col]!))
