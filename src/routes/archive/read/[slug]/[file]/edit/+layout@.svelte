			...
			<section class="flex flex-1 items-center justify-center px-4 py-16">
				<div class="w-full max-w-lg border border-[var(--archive-border)] bg-[var(--archive-paper)] p-8 text-center shadow-sm">
					<a href="/archive" class="inline-block text-[var(--archive-text)]">
						...
						<span class="mt-1 block font-[var(--font-archive-jp)] text-[13px] text-[var(--archive-faint-text)]">アイヌモシㇼ資料記録庫</span>
					</a>
					<BilingualLabel tag="h1" stacked ja={archiveLabels.accessChanged.ja} en={archiveLabels.accessChanged.en} class="mt-6 text-[27px] font-semibold" />
					<p class="mt-3 text-[15px] leading-7 text-[var(--archive-subtle)]">Reload the archive to refresh your current role and session state.</p>
					<button type="button" onclick={() => location.reload()} class="mt-6 border border-[var(--archive-gilt)] bg-[var(--archive-gilt)] px-4 py-2 text-[15px] font-semibold text-[var(--archive-paper)] hover:bg-[var(--archive-gilt-text)]">Reload</button>
				</div>
			</section>
			<ArchiveFooter />
		</div>
