...
export async function updateSourceViaMerge(
	...
	// Make the engine attach to THIS source (repo_path_exact), not fork on a
	// substantive field change. Must exist BEFORE the merge so the lookup hits.
	await ensureEditIdentifier(db, id);
	const beforeRevisionIds = new Set(
		(await db.select({ id: sourceRevisions.id }).from(sourceRevisions).where(eq(sourceRevisions.sourceId, id))).map(
			(r) => r.id
		)
	);
	...
	const result = await mergeSourceObservation(db, {
		...
		explicitDeletes,
		links: toLinkInputs(input),
		identifiers: [{ kind: 'repo_path', value: editIdentifierValue(id) }],
		actor: user.id ?? null
	});
