	...
	<div class="grid gap-3 md:grid-cols-[1.5fr_1fr_0.8fr_0.8fr_auto] md:items-end">
		...
		<div class="flex items-center gap-3">
			...
			<button type="submit" class="border border-[var(--archive-gilt)] bg-[var(--archive-gilt)] px-3 py-2 text-[13px] font-semibold text-[var(--archive-paper)] hover:bg-[var(--archive-gilt-text)] [--archive-label-en-color:var(--archive-paper)]">
				<BilingualLabel ja={archiveLabels.apply.ja} en={archiveLabels.apply.en} />
			</button>
			<a href={archiveFilterHref('/archive', { searchableOnly: false, sort: 'updated' })} class="text-[13px] text-[var(--archive-gilt-text)] hover:text-[var(--archive-gilt)]">
