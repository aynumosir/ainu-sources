									...
									{:else}
										...
										{#if roleFromValue(selectedRoles[user.userId]) !== user.role}
											<button
												...
												onclick={() => save(user)}
												class="h-8 border border-[var(--archive-accent)] bg-[var(--archive-accent)] px-3 text-[13px] font-semibold text-[var(--archive-paper)] hover:bg-[var(--archive-accent-hover)] disabled:opacity-60"
											>
