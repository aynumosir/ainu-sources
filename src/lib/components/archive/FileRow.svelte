...
<div class="border border-[var(--archive-border)] bg-[var(--archive-paper)] p-4">
	<div class="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
		...
		<div class="flex gap-2">
			{#if file.revisionId}
				<a href={`/archive/read/${sourceSlug}/${file.fileId}`} aria-label={bilingualAriaLabel(archiveLabels.read)} class="border border-[var(--archive-border)] bg-[var(--archive-paper)] px-3 py-2 text-[13px] hover:border-[var(--archive-accent)]">
					<BilingualLabel ja={archiveLabels.read.ja} en={archiveLabels.read.en} />
				</a>
				<button type="button" aria-label={bilingualAriaLabel(archiveLabels.download)} onclick={() => download?.open()} class="border border-[var(--archive-accent)] bg-[var(--archive-accent)] px-3 py-2 text-[13px] font-semibold text-[var(--archive-paper)] hover:bg-[var(--archive-accent-hover)]">
